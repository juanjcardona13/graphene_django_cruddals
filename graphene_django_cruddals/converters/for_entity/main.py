...
def convert_django_model_to_object_type(
    ...
    MetaType = build_class(
        name="Meta", attrs={"model": model, "registry": registry, **meta_attrs}
    )
    ModelObjectType = build_class(
        name=f"{singular_camel_case_name}Type",
        bases=(DjangoObjectType,),
        attrs={"Meta": MetaType, **attr_output_fields, **extra_attrs},
    )

    registry.register_model(
        model, TypeRegistryForModelEnum.OBJECT_TYPE.value, ModelObjectType
    )
    return ModelObjectType


def convert_django_model_to_paginated_object_type(
    model: DjangoModel,
    ...
    if model_as_object_type is None:
        model_as_object_type = convert_django_model_to_object_type(
    ...
    registry.register_model(
        model, TypeRegistryForModelEnum.PAGINATED_OBJECT_TYPE.value, ModelPaginatedType
    )
    return ModelPaginatedType


def convert_django_model_to_mutate_input_object_type(
    model: DjangoModel,
    registry: RegistryGlobal,
    ...
    attr_input_fields = get_input_fields_for_create_update(
        model, registry, type_mutation, meta_attrs
    )
    attrs_final = convert_args_type_relation(
        model, {**attr_input_fields, **extra_attrs}, registry, type_mutation
    )
...
def convert_django_model_to_filter_input_object_type(
    ...
    attr_input_fields = get_input_fields_for_filter(
        model, registry, meta_attrs=meta_attrs
    )
    attr_input_fields.update(
        {
            ...
            "NOT": graphene.Dynamic(
                lambda: graphene.InputField(
                    convert_django_model_to_filter_input_object_type(model, registry)
                )
            ),
        }
    )
    ModelInputObjectType = build_class(
        name=f"Filter{singular_camel_case_name}Input",
        bases=(graphene.InputObjectType,),
        attrs={**attr_input_fields, **extra_attrs},
    )
    registry.register_model(
        model,
...
def convert_django_model_to_order_by_input_object_type(
    ...
    attr_input_fields = get_input_fields_for_order_by(
        model, registry, meta_attrs=meta_attrs
    )
    ModelInputObjectType = build_class(
        name=f"OrderBy{singular_camel_case_name}Input",
        bases=(graphene.InputObjectType,),
        attrs={**attr_input_fields, **extra_attrs},
    )
    registry.register_model(
