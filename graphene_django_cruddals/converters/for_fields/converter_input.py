...
@singledispatch
def convert_django_field_to_input(field: DjangoField, registry: RegistryGlobal) -> Union[graphene.InputField, None]: #String, Upload, Int, Boolean, BigInt, Date, Time, DateTime, Decimal, Float, Duration, Binary, JSONString, UUID, Email, IPv4, IP, PositiveInt, Slug, URL
    raise Exception(
...
def convert_field_to_id(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.ID, description=get_django_field_description(field), required=is_required(field))
...
def convert_field_to_string(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.String, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.FileField)
@convert_django_field_to_input.register(models.ImageField)
def convert_field_to_upload_or_string(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=Upload, description=get_django_field_description(field), required=is_required(field))
...
def convert_field_to_int(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.Int, description=get_django_field_description(field), required=is_required(field))


# @convert_django_field_to_input.register(models.NullBooleanField)
@convert_django_field_to_input.register(models.BooleanField)
def convert_field_to_boolean(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.Boolean, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.BigIntegerField)
def convert_field_to_big_int(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.BigInt, description=get_django_field_description(field), required=is_required(field))
@convert_django_field_to_input.register(models.DateField)
def convert_field_to_date(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.Date, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.TimeField)
def convert_field_to_time(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.Time, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.DateTimeField)
def convert_field_to_datetime(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.DateTime, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.DecimalField)
def convert_field_to_decimal(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.Decimal, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.FloatField)
def convert_field_to_float(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.Float, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.DurationField)
def convert_field_to_duration(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=Duration, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.BinaryField)
def convert_field_to_binary(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=Binary, description=get_django_field_description(field), required=is_required(field))
...
def convert_pg_and_json_field_to_json_string(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.JSONString, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.UUIDField)
def convert_field_to_uuid(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=graphene.UUID, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.EmailField)
def convert_field_to_email(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=Email, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.GenericIPAddressField)
def convert_field_to_ipv4(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=IPv4, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.IPAddressField)
def convert_field_to_ip(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=IP, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.PositiveIntegerField)
def convert_field_to_positive_int(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=PositiveInt, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.SlugField)
def convert_field_to_slug(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=Slug, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.URLField)
def convert_field_to_url(field:DjangoField, registry: RegistryGlobal):
    return InputField(type_=URL, description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.OneToOneRel)
def convert_onetoone_field_to_djangomodel(field:DjangoField, registry: RegistryGlobal):
    
    # from graphene_django_cruddals.converters.for_entity.utils import converter_pk_field
    ...
    # if not pk_field:
    #     return ID(required=not field.null)
    # converted_pk_field = converter_pk_field(pk_field, registry)
    # if not converted_pk_field:
    #     return ID(required=not field.null)
    # return converted_pk_field
    return InputField(type_=ID, description=get_django_field_description(field), required=is_required(field))
...
def convert_field_to_list_or_connection(field:DjangoField, registry: RegistryGlobal):
    ...
    # converted_pk_field = converter_pk_field(pk_field, registry)
    # if not converted_pk_field:
    #     return List(ID, required=is_required(field))
    # return List(converted_pk_field.__class__, required=is_required(field))

    return InputField(type_=List(ID), description=get_django_field_description(field), required=is_required(field))


@convert_django_field_to_input.register(models.OneToOneField)
@convert_django_field_to_input.register(models.ForeignKey)
def convert_field_to_djangomodel(field:DjangoField, registry: RegistryGlobal):
    
    # model = field.related_model
    # pk_field = model._meta.pk
    # converted_pk_field = converter_pk_field(pk_field, registry)
    # if not converted_pk_field:
    #     return ID(required=is_required(field))
...
def convert_field_to_union_type(field:DjangoField, registry: RegistryGlobal):
    return


@convert_django_field_to_input.register(GenericRelation)
def convert_relation_field_to_union_type(field:DjangoField, registry: RegistryGlobal):
    return


@convert_django_field_to_input.register(GenericRel) # Representa otro tipo OneToMany
def convert_field_to__type(field:DjangoField, registry: RegistryGlobal):
    """TODO Add support"""
    return
