...
def convert_field_to_id(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.ID, description=get_django_field_description(field), required=not field.blank)
...
def convert_field_to_string(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.String, description=get_django_field_description(field), required=not field.blank)
...
def convert_field_to_int(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.Int, description=get_django_field_description(field), required=not field.blank)
...
def convert_field_to_boolean(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.Boolean, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.BigIntegerField)
def convert_field_to_big_int(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.BigInt, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.DateField)
def convert_field_to_date(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.Date, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.TimeField)
def convert_field_to_time(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.Time, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.DateTimeField)
def convert_field_to_datetime(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.DateTime, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.DecimalField)
def convert_field_to_decimal(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.Decimal, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.FloatField)
def convert_field_to_float(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.Float, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.DurationField)
def convert_field_to_duration(field, registry: RegistryGlobal):
    return graphene.Field(type_=Duration, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.BinaryField)
def convert_field_to_binary(field, registry: RegistryGlobal):
    return graphene.Field(type_=Binary, description=get_django_field_description(field), required=not field.blank)
...
def convert_pg_and_json_field_to_json_string(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.JSONString, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.UUIDField)
def convert_field_to_uuid(field, registry: RegistryGlobal):
    return graphene.Field(type_=graphene.UUID, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.EmailField)
def convert_field_to_email(field, registry: RegistryGlobal):
    return graphene.Field(type_=Email, description=get_django_field_description(field), required=not field.blank)
...
def convert_field_to_ipv4(field, registry: RegistryGlobal):
    return graphene.Field(type_=IPv4, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.IPAddressField)
def convert_field_to_ip(field, registry: RegistryGlobal):
    return graphene.Field(type_=IP, description=get_django_field_description(field), required=not field.blank)
...
def convert_field_to_positive_int(field, registry: RegistryGlobal):
    return graphene.Field(type_=PositiveInt, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.SlugField)
def convert_field_to_slug(field, registry: RegistryGlobal):
    return graphene.Field(type_=Slug, description=get_django_field_description(field), required=not field.blank)


@convert_django_field_to_output.register(models.URLField)
def convert_field_to_url(field, registry: RegistryGlobal):
    return graphene.Field(type_=URL, description=get_django_field_description(field), required=not field.blank)
...
def convert_onetoone_field_to_djangomodel(field, registry: RegistryGlobal):
    ...
    def dynamic_type():
        ...
        return graphene.Field(related_type, required=not field.null, resolver=default_resolver)
