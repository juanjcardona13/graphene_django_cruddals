...
from django.db.models import OneToOneField, OneToOneRel, Q
...
def update_dict_with_model_instance(obj_to_update:dict, instance:Union[Type[DjangoModel], None]=None, model:Union[DjangoModel, None]=None) -> dict:
    """
...
def toggle_active_status(option:Union[Literal['ACTIVATE', 'DEACTIVATE'], Literal['activate', 'deactivate']], data:DjangoQuerySet, field:str="is_active") -> DjangoQuerySet:
    """
...
def get_list_input_object_type(value_of_field_to_convert, model):
    is_generic_foreign_key = isinstance(value_of_field_to_convert, GenericForeignKeyInput)
    if is_generic_foreign_key:
        ...
        value_of_field_to_convert = {to_snake_case(key): value for key, value in value_of_field_to_convert.object.items()}
    ...
    if is_list_of_same_type(list_values, graphene.InputObjectType) or is_generic_foreign_key:
        list_values_to_modify = [
...
def get_resolvers_for_field(model, registry):
    ...
    if registries_for_model:
        ...
        if cruddals_for_related_model:
            create_field_resolver = cruddals_for_related_model.meta.create_field.resolver
            update_field_resolver = cruddals_for_related_model.meta.update_field.resolver
    ...
    for obj_to_relate in list_objects_to_relate:
        if isinstance(field_relation, (ManyToOneRel, OneToOneRel)):
            ...
            if getattr(obj_to_relate, direct_field_detail["pk_field_name"], None):
                reverse_pk_value = getattr( obj_to_relate, direct_field_detail["pk_field_name"] )
                reverse_actual_obj = direct_field_detail["model"].objects.get( pk=reverse_pk_value )
                query_set_reverse_actual_objs: DjangoQuerySet = getattr( reverse_actual_obj, direct_field_detail["name_field"] ).all()
            ...
            content_type = ContentType.objects.get(app_label=name_app_inverse, model=name_model_inverse)
        ...
        if isinstance( direct_field_detail["field"], (ManyToManyField, ManyToManyRel) ):
            ...
            for obj_to_disconnect in list_values_to_disconnect:
                if direct_field_detail["field"].field.null:
                    obj_to_disconnect.__setattr__( direct_field_detail["field"].field.attname, None )
                    obj_to_disconnect.save()
                elif direct_field_detail["field"].field.blank:
                    obj_to_disconnect.__setattr__( direct_field_detail["field"].field.attname, "" )
                    obj_to_disconnect.save()
                else:
            ...
            direct_pk_value = obj_to_modify[direct_field_detail["pk_field_name"]]
            direct_actual_obj = direct_field_detail["field"].related_model.objects.get( pk=direct_pk_value )
            name_field_generic_relation = direct_field_detail["field"].field.name
            getattr(direct_actual_obj, name_field_generic_relation).remove( *list_values_to_disconnect )
...
def get_relation_field_details(type_field_relation, django_relation_field, value_of_field):
    if type_field_relation == "field_direct":
        ...
        if isinstance(django_relation_field, GenericForeignKey):
            generic_type = ContentType.objects.get(app_label=value_of_field.app_label, model=value_of_field.model)
            ...
            reverse_name_field = django_relation_field.related_query_name() if hasattr(django_relation_field, 'related_query_name') else None
            reverse_pk_field_name = reverse_model._meta.pk.name if reverse_model else None
...
def create_relation_model_objects(
    ...
    valid_relations = ( (ManyToManyField, ForeignKey, OneToOneField, GenericForeignKey) if type_field_relation == "field_direct" else (ManyToManyRel, ManyToOneRel, OneToOneRel, GenericRelation) )
    fields_to_remove = []
    responses = {}
    for name_field, value_of_field in obj_to_modify.items():
        django_field = all_model_fields.get(name_field, None)
        if ( django_field and django_field.is_relation and isinstance(django_field, valid_relations) and value_is_input_object_type(value_of_field)):
            relation_field_details = get_relation_field_details( type_field_relation, django_field, value_of_field )
            ...
            mutate_for_create, mutate_for_update = get_resolvers_for_field( model_of_django_field, registry )
            ...
            if ( "input_object_type_for_connect_disconnect" in registries_for_model_of_django_field ):
                ...
                if isinstance(value_of_field, input_object_type_for_connect_disconnect) or value_of_field.__class__.__name__ == input_object_type_for_connect_disconnect.__name__:

                    field_details = ( relation_field_details["direct"] if type_field_relation == "field_direct" else relation_field_details["reverse"] )

                    handle_disconnect_objs_related( field_details, model_of_django_field, value_of_field, obj_to_modify, )  # TODO, Que pasa si se presenta un error??
            ...
            if ( "input_object_type_for_connect" in registries_for_model_of_django_field ):
                input_object_type_for_connect = ( registries_for_model_of_django_field[ "input_object_type_for_connect" ] )
                if isinstance(value_of_field, input_object_type_for_connect) or value_of_field.__class__.__name__ == input_object_type_for_connect.__name__:
                    if "connect" in value_of_field and value_of_field["connect"]:
                        value_of_field = value_of_field["connect"]
...
def get_object_type_payload(model_object_type, name_for_output_type, include_success=False):
    ...
    return build_class(name=name_for_output_type, bases=(graphene.ObjectType,), attrs=output_fields)
